...
@Singleton
internal class SteamLoopbackTlsBridge @Inject constructor(
    ...
    private fun readConnectRequest(socket: Socket): ConnectRequest {
        ...
        while (output.size() < MAX_HEADER_BYTES && matched < HEADER_TERMINATOR.size) {
            ...
            output.write(value)
            matched = if (value.toByte() == HEADER_TERMINATOR[matched]) matched + 1 else if (value == '\r'.code) 1 else 0
        }
        ...
        val headers = lines.drop(1).mapNotNull { line ->
            ...
            if (separator <= 0) null else line.substring(0, separator).trim().lowercase() to
