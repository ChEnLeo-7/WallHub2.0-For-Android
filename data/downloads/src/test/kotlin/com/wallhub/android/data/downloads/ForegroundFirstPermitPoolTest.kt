...
class ForegroundFirstPermitPoolTest {
    @Test
    fun `foreground stream reads retain a reserved permit and pass queued prefetch`() = runTest {
        ...
        val firstPrefetch = async(start = CoroutineStart.UNDISPATCHED) {
            pool.withPermit(SteamStreamChunkPriority.PREFETCH) { firstPrefetchRelease.await() }
        }
        val secondPrefetch = async(start = CoroutineStart.UNDISPATCHED) {
            pool.withPermit(SteamStreamChunkPriority.PREFETCH) { secondPrefetchRelease.await() }
        }
        val queuedPrefetch = async(start = CoroutineStart.UNDISPATCHED) {
        ...
        val queuedForeground = async(start = CoroutineStart.UNDISPATCHED) {
