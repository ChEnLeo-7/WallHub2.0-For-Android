...
@Singleton
class FileDiagnosticRepository @Inject constructor(
    ...
    override suspend fun readRecent(limit: Int): List<DiagnosticEvent> = withContext(Dispatchers.IO) {
    ...
    override suspend fun exportRedactedText(): String = withContext(Dispatchers.IO) {
        writeMutex.withLock {
    ...
    private fun DiagnosticEvent.toLogLine(): String = listOf(
        timestamp.toString(),
        level.name,
    ...
    private fun String.cleanLogField(): String = replace('\t', ' ')
        .replace('\r', ' ')
        .replace('\n', ' ')
...
internal object DiagnosticSanitizer {
    private val sensitiveAttributeNames = setOf(
        "password",
        "passwd",
        "pwd",
        "token",
    ...
    private val authorizationBearerPattern = Regex(
    ...
    fun sanitize(event: DiagnosticEvent): DiagnosticEvent = event.copy(
        ...
        attributes = event.attributes.mapValues { (key, value) ->
    ...
    private fun String.normalizedDiagnosticKey(): String = trim()
        .lowercase()
        .replace('-', '_')
