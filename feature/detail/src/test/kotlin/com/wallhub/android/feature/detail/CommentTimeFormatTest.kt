...
class CommentTimeFormatTest {
    ...
    private val now = LocalDateTime.of(2026, 7, 15, 12, 0, 0)
        .atZone(zone)
        .toInstant()
        .toEpochMilli()
    ...
    fun `older comments omit only the current year`() {
        val sameYear = LocalDateTime.of(2026, 6, 11, 15, 4, 3)
            .atZone(zone)
            .toEpochSecond()
        val priorYear = LocalDateTime.of(2025, 6, 11, 15, 4, 3)
            .atZone(zone)
            .toEpochSecond()
    ...
    private fun commentAt(timestampMillis: Long) = WorkshopComment(
        author = "A",
        text = "B",
