...
- 基本设置将 WallHub 应用信息、作者与贡献人员、QQ 社群、官方仓库和版本更新整合为一个 Material 3 板块：QQ 群号修正为 `1082323527`，作者和三位贡献人员显示 GitHub 头像及具体职责；检查更新收敛为行尾图标，Release 说明默认只显示单一入口，点击后在可滚动弹窗中完整渲染 Markdown。下载、取消与系统安装按钮仅在对应状态出现；NSFW 内容开关移动到基本设置最底部。更新 APK 仍须通过 SHA-256、包名和安装证书校验，并交由 Android 系统安装器确认，不会静默安装、绕过未知来源授权或清除现有数据。
