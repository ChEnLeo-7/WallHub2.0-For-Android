...
- Android 全局 Steam 流量加速方案整理为本地设计文档：使用系统 `VpnService`、TUN 和成熟用户态 TCP/UDP 转发引擎，只改变识别出的 Steam 流量；兼容 OpenClash Fake-IP，Community 可按 host 使用合法 TLS Record Fragmentation，Web API/CM/CDN 保持直连，并禁止 CA 安装、TLS 解密、SNI 替换和证书校验降级。
