...
- 基本设置将 WallHub 应用信息、作者与贡献人员、QQ 社群、官方仓库和版本更新整合为一个 Material 3 板块；板块标题更新为 `WallHub For Android`，顶部使用项目大 LOGO、居中标题、版本、APK 更新日期与简短介绍，不再显示包名。QQ 群号为 `1082323527`，整行改为打开 QQ 加群卡片，不再复制群号；作者和三位贡献人员继续显示 GitHub 头像及具体职责。检查更新收敛为行尾图标，Release 说明默认只显示单一入口，点击后在可滚动弹窗中完整渲染 Markdown。下载、取消与系统安装按钮仅在对应状态出现；NSFW 内容开关位于基本设置最底部。更新 APK 仍须通过 SHA-256、包名和安装证书校验，并交由 Android 系统安装器确认，不会静默安装、绕过未知来源授权或清除现有数据。
- Steam IP 预热与下拉刷新指示器改为同一互斥显示策略；预热期间会从 Compose 树中完全移除下拉刷新 Indicator，只保留中央预热进度环，避免状态切换动画短暂叠出两个转圈。
- 发现页 Steam IP 预热状态现在只显示中央进度指示器，不再与下拉刷新指示器重复；筛选抽屉移除强制全高 Pager，按当前页内容动态收缩并在超过可用视口时页内滚动，修复“内容”页底部大面积留白。
- 发现页筛选页面切换改为旧页先淡出、再调整动态高度并淡入新页，过渡内容始终裁剪在底表内，避免短页切换时旧/新元素与底表位移叠画；发现页和管理页共用的 FilterChip 永久保留固定勾选槽，只对勾选执行渐显、渐隐与缩放，不再因前导图标出现/消失改变 Chip 宽度或挤压相邻项。
...
- 修复设置页点击“重新检测线路”时 `ConnectionPool.evictAll()` 在 Compose 主线程关闭 TLS socket、触发 `NetworkOnMainThreadException` 并使应用崩溃的问题。手动刷新、网络回调和设置变更现在统一投递到 IO 作用域的 conflated Channel，由单一消费者串行执行清缓存、关闭连接与预热探测，刷新风暴只保留一个待处理请求且异常不会再穿透 UI 调用栈。
...
- Android 全局 Steam代理开始进入实现：新增 `:data:vpn`，固定使用 MPL-2.0 Firestack `8134609734` 的四 ABI AAR提供 TUN/TCP/UDP/DNS/ICMP用户态网络栈；全设备 `VpnService`配置 IPv4/IPv6默认路由和虚拟 DNS，所有 TCP通过本机 SOCKS5 direct relay并在 upstream `connect()` 前执行 `VpnService.protect()`，UDP/ICMP由 Firestack直接转发。纯 Kotlin ClientHello parser只对 `steamcommunity.com`及子域重封装 TLS records，保持 handshake bytes、原始 SNI和端到端证书校验不变；Firestack自身 split/retry显式关闭。实验设置删除旧内置 DoH/Hosts连接增强，替换为系统 VPN授权、全局开关、活动 flow、分片次数和 engine build状态；旧 `steamAccessEnabled`偏好在读取和写入时强制关闭。加入 fragmenter、已分片输入、trailing record、malformed input及 SOCKS5协议测试，并记录 Firestack第三方许可证。
