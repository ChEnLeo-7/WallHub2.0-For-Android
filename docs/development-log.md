...
- Android 全局 Steam 流量加速方案整理为本地设计文档：使用系统 `VpnService`、TUN 和成熟用户态 TCP/UDP 转发引擎，只改变识别出的 Steam 流量；系统 DNS 地址统一作为不透明的 `SYSTEM_RESOLVED_ROUTE` 原样交给网关，不要求判断它是真实地址还是 Fake-IP，因此兼容用户自定义的任意 IPv4/IPv6 地址池且不硬编码 `198.18.0.0/15`，用户 CIDR 或网关 API 仅提供诊断标签。Community 可按 host 使用合法 TLS Record Fragmentation，Web API/CM/CDN 保持直连，并禁止 CA 安装、TLS 解密、SNI 替换和证书校验降级；该免外部代理绕过属于待真机差分验证的经验性能力，不保证对会跨 TLS Record 重组 ClientHello 的 DPI 有效。
