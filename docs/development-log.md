...
- Fix: Match DepotDownloader's CDN authentication order: manifest and chunk requests now start without a CDN token and request one only after an HTTP 401/403 challenge. This avoids attaching an unnecessary host token that several China HTTP edges reject, while retaining authenticated retry for depots that actually require one.
...
### SteamPipe protocol alignment: server directory semantics, authorization order, and direct file_url downloads

- Fix: Restored Steam server-directory semantics after a pinned-source audit (SteamKit2 2e9b82ac, DepotDownloader c124842e, SteamDatabase protobufs f740c7db; findings in `docs/steam-workshop-download-protocol-audit.md`). CDN request URLs now honor the advertised transport exactly: `https_support=mandatory` uses HTTPS/443, everything else uses HTTP/80, and Steam content proxies keep their own advertised protocol/VHost/port/path template. The process-global HTTPS-to-HTTP fallback (`SteamCdnTransportFallback`) was removed; it was never upstream behavior and could send CDN auth tokens over cleartext HTTP.
- Fix: Authorization now matches upstream ordering. An initial HTTP 403 requests a `GetCDNAuthToken` for `(depotId, Host)` and retries the same server and protocol; 401 never requests a token; a rejected token is invalidated and force-refreshed at most once per server; authorization failures are no longer classified as recoverable transport errors. Tokens are requested for both anonymous and authenticated sessions and the cache gains generation-safe invalidation/refresh.
- Fix: Workshop identity no longer assumes `depotId == appId`. A PICS `CMsgClientPICSProductInfoRequest` (with access-token fallback) plus SteamKit2-compatible text KeyValues parsing resolves the real `depots/workshopdepot`, carried through `SteamContentAccess` into depot keys, request codes, tokens, manifests, and chunks. Manifest request code zero is now an explicit access failure and manifest URLs always embed a nonzero uint64 request code. Parsed manifests are validated against the requested depot ID and manifest GID.
- Fix: CDN server selection filters entries by `SteamCache`/`CDN` type, `allowed_app_ids`, and proxy role; ordering is weighted-load only (no HTTPS-first re-ranking). Chunk download now retries fetch + decode + decompressed-length + Adler-32 checksum as one operation per server for both full downloads and streaming, and selector success is recorded only after full verification.
- Update: Formal downloads implement published-file semantics: a non-empty `file_url` takes precedence over SteamPipe and streams directly (HTTPS, range resume with exact Content-Range validation, `.wallhub.part` staging, sanitized filename) into a conversion-ready video staging set (video + validated preview + synthetic project.json). Direct downloads skip Steam transport leases and account gating; preview URLs are persisted with the task.
- Update: CDN protocol tests rewritten (`SteamCdnHttpsTest` covers advertised HTTP/HTTPS/proxy transports and request-code contract; former fallback tests replaced by eligibility/403-challenge tests; direct-file naming, Content-Range, and preview-signature tests added).
- Verification: pending - commit-bound GitHub Actions verify run (unit tests, lint budget, detekt, ktlint, signed release) plus OnePlus 5T install and logged-in retest of previously failing Workshop items.

### SteamPipe protocol alignment: device verification results

- Verification: Commits `7d0453f`..`bde5566` green on GitHub Actions (unit tests, lint budget, detekt, ktlint, signed release) and installed on OnePlus 5T (arm64) with data preserved. Logged-in isolated retests on the degraded-CN-CDN network: `Miku Audio Visualizer 3779918497`, `Lost Valley 2 3774904326`, and `Remielle Dan 3786889734` - all previously failing at the manifest stage for days - now download and convert successfully with full chunk checksum verification. All test tasks and exports were removed afterwards; `/sdcard/Download/WallHub` retains only the five pre-existing Sep-5 files; cold-start PID stable, zero FATAL/ANR/OOM.
- Fix: PICS AppInfo `buffer` is TEXT KeyValues with one trailing NUL (SteamKit2 parses it with `KeyValue.ReadAsText`); kxvdf's root skipper could not map the dynamic app-ID root, so `workshopdepot` is now extracted with a small deterministic tokenizer that walks quoted tokens and braces and accepts the value directly under the root or under the `appinfo` wrapper.
- Fix: Steam manifest chunk CRCs follow SteamKit2's `DepotChunk.AdlerHash` (s1 seeded with 0), not RFC 1950. The Java helper used `java.util.zip.Adler32` (seeded 1), which made freshly downloaded chunks fail verification while the zero-seeded Rust engine accepted the same bytes; the Kotlin checksum now matches the Steam convention for byte and streaming inputs, which also makes resume-time chunk verification meaningful.
- Fix: `hcontent_file` is a uint64 that already exceeds `Long.MAX_VALUE` for recent items; Android's JSON parser degrades such literals, so the raw published-file details body is matched and the digits are reinterpreted as unsigned bits of a Long, with manifest handles formatted unsigned in request-code errors and CDN manifest URLs. Nonzero raw-bit handles are treated as present.
- Fix: A same-account Webview/DepotDownloader control test proved `予愿安洁莉娜 3775650263` downloads successfully from `xz.pphimalayanrt.com:80` using manifest `6566827351488351196` (18,331,320 bytes), while Android stopped before CDN. The response code was not zero: `manifest_request_code` is a uint64 whose high bit may make the generated Kotlin `Long` negative. Android's `> 0` checks misclassified that valid raw-bit value as denial. Request-code presence now means `!= 0`, and the URL formats its Long bits with `toULong()`.
