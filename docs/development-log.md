...
- Steam 服务访问新增 DoH 地址与优先级编辑器：保留现有默认列表，支持添加自定义 HTTPS DoH、上移、下移、删除和恢复默认，按用户顺序优先采用解析结果；保存时统一执行 URL 安全校验、去重和八项上限，并一次性清理旧线路后重新检测。编辑器按 Material 3 自适应：紧凑窗口使用近全高 Modal Bottom Sheet，中等以上窗口使用居中 Dialog，标题、滚动列表与操作区独立；采用标准 ListItem、优先级徽标、OutlinedTextField、48dp 操作目标、IME 提交、固定保存区和未保存更改确认。
