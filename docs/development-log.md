...
- Fix: Workshop identity no longer assumes `depotId == appId`. A PICS `CMsgClientPICSProductInfoRequest` (with access-token fallback) plus binary VDF decode resolves the real `depots/workshopdepot`, carried through `SteamContentAccess` into depot keys, request codes, tokens, manifests, and chunks. Manifest request code zero is now an explicit access failure and manifest URLs always embed a positive request code. Parsed manifests are validated against the requested depot ID and manifest GID.
...
- Behavior note: `予愿安洁莉娜 3775650263` consistently receives `manifest_request_code = 0` from Steam (`Steam denied manifest request code ...`), so the download now stops with that explicit access failure instead of silently issuing a no-code manifest URL (the old behavior produced opaque 401s). This matches DepotDownloader's zero-code handling; the item becomes downloadable again once Steam grants a code.
