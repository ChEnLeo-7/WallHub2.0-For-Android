...
# The LAN worker keeps its checkout between jobs; remove stale native and
# Gradle intermediates before publishing the libraries for this build.
rm -rf "$JNI_LIBS"
...
export CXX_aarch64_linux_android="$TOOLCHAIN_BIN/aarch64-linux-android${MIN_SDK_API}-clang++$LINKER_SUFFIX"
export CXX_armv7_linux_androideabi="$TOOLCHAIN_BIN/armv7a-linux-androideabi${MIN_SDK_API}-clang++$LINKER_SUFFIX"
export CXX_i686_linux_android="$TOOLCHAIN_BIN/i686-linux-android${MIN_SDK_API}-clang++$LINKER_SUFFIX"
export CXX_x86_64_linux_android="$TOOLCHAIN_BIN/x86_64-linux-android${MIN_SDK_API}-clang++$LINKER_SUFFIX"
